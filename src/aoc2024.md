...
(defun check-pairs (sequence &optional (comparison-func #'<=))
  (loop for i from 1 below (length sequence) do
    (unless (funcall comparison-func (nth (1- i) sequence) (nth i sequence)) (return-from check-pairs nil)))
  t)
...
most three."
  (cond ((< (first sequence) (second sequence))
	 (check-pairs sequence #'(lambda (x y) (and (<= x y) (pair-distance-ok-p x y)))))
